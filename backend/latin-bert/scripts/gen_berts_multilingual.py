...
default_tokenizer_path = os.path.join(script_dir, '..', 'models', 'subword_tokenizer_latin', 'latin.subword.encoder')
...
class LatinBERT():
    ...
    def get_batches(self, sentences, max_batch, tokenizer):
        ...
        while i < len(ordered_data):
            ...
            batch_transforms_np = np.array(batch_transforms)
            # Convert the numPy array to a PyTorch tensor and get its shape
            batched_transforms.append(torch.FloatTensor(batch_transforms_np))
            # batched_transforms.append(torch.FloatTensor(batch_transforms))

            bsize = torch.FloatTensor(batch_transforms_np).shape
    ...
    def get_berts(self, raw_sents):
        ...
        batch_size = 32
        # Setting the Bert model to evaluation mode
        self.model.eval()
        batched_data, batched_mask, batched_transforms, ordering = self.get_batches(sents, batch_size,
                                                                                    self.wp_tokenizer)
        ...
        for b in range(len(batched_data)):
            ...
            b_size = size[0]
            berts = self.model.forward(batched_data[b], attention_mask=batched_mask[b],
...
if __name__ == "__main__":
    ...
    parser.add_argument('-t', '--tokenizerPath', help='path to Latin WordPiece tokenizer', required=False,
